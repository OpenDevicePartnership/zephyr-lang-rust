#include "/home/blake/Repos/zephyr/zephyr-rust/modules/lang/rust/zephyr-sys/wrapper.h"
...
void sys_dlist_range_prepend__extern(sys_dlist_t *dest, sys_dnode_t *start, sys_dnode_t *last) { sys_dlist_range_prepend(dest, start, last); }
void sys_dlist_range_append__extern(sys_dlist_t *dest, sys_dnode_t *start, sys_dnode_t *last) { sys_dlist_range_append(dest, start, last); }
...
void device_map__extern(mm_reg_t *virt_addr, uintptr_t phys_addr, size_t size, uint32_t flags) { device_map(virt_addr, phys_addr, size, flags); }
void device_unmap__extern(mm_reg_t virt_addr, size_t size) { device_unmap(virt_addr, size); }
...
status_t EnableIRQ__extern(IRQn_Type interrupt) { return EnableIRQ(interrupt); }
status_t EnableIRQWithPriority__extern(IRQn_Type interrupt, uint8_t priNum) { return EnableIRQWithPriority(interrupt, priNum); }
void EnableGlobalIRQ__extern(uint32_t primask) { EnableGlobalIRQ(primask); }
...
int k_object_access_check__extern(const void *object) { return k_object_access_check(object); }
...
int uart_err_check__extern(const struct device *dev) { return uart_err_check(dev); }
int uart_poll_in__extern(const struct device *dev, unsigned char *p_char) { return uart_poll_in(dev, p_char); }
int uart_poll_in_u16__extern(const struct device *dev, uint16_t *p_u16) { return uart_poll_in_u16(dev, p_u16); }
void uart_poll_out__extern(const struct device *dev, unsigned char out_char) { uart_poll_out(dev, out_char); }
void uart_poll_out_u16__extern(const struct device *dev, uint16_t out_u16) { uart_poll_out_u16(dev, out_u16); }
int uart_configure__extern(const struct device *dev, const struct uart_config *cfg) { return uart_configure(dev, cfg); }
int uart_config_get__extern(const struct device *dev, struct uart_config *cfg) { return uart_config_get(dev, cfg); }
int uart_fifo_fill__extern(const struct device *dev, const uint8_t *tx_data, int size) { return uart_fifo_fill(dev, tx_data, size); }
int uart_fifo_fill_u16__extern(const struct device *dev, const uint16_t *tx_data, int size) { return uart_fifo_fill_u16(dev, tx_data, size); }
int uart_fifo_read__extern(const struct device *dev, uint8_t *rx_data, const int size) { return uart_fifo_read(dev, rx_data, size); }
int uart_fifo_read_u16__extern(const struct device *dev, uint16_t *rx_data, const int size) { return uart_fifo_read_u16(dev, rx_data, size); }
void uart_irq_tx_enable__extern(const struct device *dev) { uart_irq_tx_enable(dev); }
void uart_irq_tx_disable__extern(const struct device *dev) { uart_irq_tx_disable(dev); }
int uart_irq_tx_ready__extern(const struct device *dev) { return uart_irq_tx_ready(dev); }
void uart_irq_rx_enable__extern(const struct device *dev) { uart_irq_rx_enable(dev); }
void uart_irq_rx_disable__extern(const struct device *dev) { uart_irq_rx_disable(dev); }
int uart_irq_tx_complete__extern(const struct device *dev) { return uart_irq_tx_complete(dev); }
int uart_irq_rx_ready__extern(const struct device *dev) { return uart_irq_rx_ready(dev); }
void uart_irq_err_enable__extern(const struct device *dev) { uart_irq_err_enable(dev); }
void uart_irq_err_disable__extern(const struct device *dev) { uart_irq_err_disable(dev); }
int uart_irq_is_pending__extern(const struct device *dev) { return uart_irq_is_pending(dev); }
int uart_irq_update__extern(const struct device *dev) { return uart_irq_update(dev); }
int uart_irq_callback_user_data_set__extern(const struct device *dev, uart_irq_callback_user_data_t cb, void *user_data) { return uart_irq_callback_user_data_set(dev, cb, user_data); }
int uart_irq_callback_set__extern(const struct device *dev, uart_irq_callback_user_data_t cb) { return uart_irq_callback_set(dev, cb); }
int uart_callback_set__extern(const struct device *dev, uart_callback_t callback, void *user_data) { return uart_callback_set(dev, callback, user_data); }
int uart_tx__extern(const struct device *dev, const uint8_t *buf, size_t len, int32_t timeout) { return uart_tx(dev, buf, len, timeout); }
int uart_tx_u16__extern(const struct device *dev, const uint16_t *buf, size_t len, int32_t timeout) { return uart_tx_u16(dev, buf, len, timeout); }
int uart_tx_abort__extern(const struct device *dev) { return uart_tx_abort(dev); }
int uart_rx_enable__extern(const struct device *dev, uint8_t *buf, size_t len, int32_t timeout) { return uart_rx_enable(dev, buf, len, timeout); }
int uart_rx_enable_u16__extern(const struct device *dev, uint16_t *buf, size_t len, int32_t timeout) { return uart_rx_enable_u16(dev, buf, len, timeout); }
int uart_rx_buf_rsp__extern(const struct device *dev, uint8_t *buf, size_t len) { return uart_rx_buf_rsp(dev, buf, len); }
int uart_rx_buf_rsp_u16__extern(const struct device *dev, uint16_t *buf, size_t len) { return uart_rx_buf_rsp_u16(dev, buf, len); }
int uart_rx_disable__extern(const struct device *dev) { return uart_rx_disable(dev); }
int uart_line_ctrl_set__extern(const struct device *dev, uint32_t ctrl, uint32_t val) { return uart_line_ctrl_set(dev, ctrl, val); }
int uart_line_ctrl_get__extern(const struct device *dev, uint32_t ctrl, uint32_t *val) { return uart_line_ctrl_get(dev, ctrl, val); }
int uart_drv_cmd__extern(const struct device *dev, uint32_t cmd, uint32_t p) { return uart_drv_cmd(dev, cmd, p); }
bool sensor_chan_spec_eq__extern(struct sensor_chan_spec chan_spec0, struct sensor_chan_spec chan_spec1) { return sensor_chan_spec_eq(chan_spec0, chan_spec1); }
int sensor_decode__extern(struct sensor_decode_context *ctx, void *out, uint16_t max_count) { return sensor_decode(ctx, out, max_count); }
int sensor_attr_set__extern(const struct device *dev, enum sensor_channel chan, enum sensor_attribute attr, const struct sensor_value *val) { return sensor_attr_set(dev, chan, attr, val); }
int sensor_attr_get__extern(const struct device *dev, enum sensor_channel chan, enum sensor_attribute attr, struct sensor_value *val) { return sensor_attr_get(dev, chan, attr, val); }
int sensor_trigger_set__extern(const struct device *dev, const struct sensor_trigger *trig, sensor_trigger_handler_t handler) { return sensor_trigger_set(dev, trig, handler); }
int sensor_sample_fetch__extern(const struct device *dev) { return sensor_sample_fetch(dev); }
int sensor_sample_fetch_chan__extern(const struct device *dev, enum sensor_channel type) { return sensor_sample_fetch_chan(dev, type); }
int sensor_channel_get__extern(const struct device *dev, enum sensor_channel chan, struct sensor_value *val) { return sensor_channel_get(dev, chan, val); }
int32_t sensor_ms2_to_g__extern(const struct sensor_value *ms2) { return sensor_ms2_to_g(ms2); }
void sensor_g_to_ms2__extern(int32_t g, struct sensor_value *ms2) { sensor_g_to_ms2(g, ms2); }
int32_t sensor_ms2_to_mg__extern(const struct sensor_value *ms2) { return sensor_ms2_to_mg(ms2); }
int32_t sensor_ms2_to_ug__extern(const struct sensor_value *ms2) { return sensor_ms2_to_ug(ms2); }
void sensor_ug_to_ms2__extern(int32_t ug, struct sensor_value *ms2) { sensor_ug_to_ms2(ug, ms2); }
int32_t sensor_rad_to_degrees__extern(const struct sensor_value *rad) { return sensor_rad_to_degrees(rad); }
void sensor_degrees_to_rad__extern(int32_t d, struct sensor_value *rad) { sensor_degrees_to_rad(d, rad); }
int32_t sensor_rad_to_10udegrees__extern(const struct sensor_value *rad) { return sensor_rad_to_10udegrees(rad); }
void sensor_10udegrees_to_rad__extern(int32_t d, struct sensor_value *rad) { sensor_10udegrees_to_rad(d, rad); }
double sensor_value_to_double__extern(const struct sensor_value *val) { return sensor_value_to_double(val); }
float sensor_value_to_float__extern(const struct sensor_value *val) { return sensor_value_to_float(val); }
int sensor_value_from_double__extern(struct sensor_value *val, double inp) { return sensor_value_from_double(val, inp); }
int sensor_value_from_float__extern(struct sensor_value *val, float inp) { return sensor_value_from_float(val, inp); }
int64_t sensor_value_to_deci__extern(const struct sensor_value *val) { return sensor_value_to_deci(val); }
int64_t sensor_value_to_centi__extern(const struct sensor_value *val) { return sensor_value_to_centi(val); }
int64_t sensor_value_to_milli__extern(const struct sensor_value *val) { return sensor_value_to_milli(val); }
int64_t sensor_value_to_micro__extern(const struct sensor_value *val) { return sensor_value_to_micro(val); }
int sensor_value_from_milli__extern(struct sensor_value *val, int64_t milli) { return sensor_value_from_milli(val, milli); }
int sensor_value_from_micro__extern(struct sensor_value *val, int64_t micro) { return sensor_value_from_micro(val, micro); }
int sensor_get_decoder__extern(const struct device *dev, const struct sensor_decoder_api **decoder) { return sensor_get_decoder(dev, decoder); }
int sensor_reconfigure_read_iodev__extern(const struct rtio_iodev *iodev, const struct device *sensor, const struct sensor_chan_spec *channels, size_t num_channels) { return sensor_reconfigure_read_iodev(iodev, sensor, channels, num_channels); }
